...
void write_array_to_file(char * choices, char * marks)
{
...
char * alternateMerge(char * arr, char * a, char * b){
	int alen = strlen(a);
	int blen = strlen(b);
	int len = alen + blen; 
	char * 
	...
	while(1){
		...
		if(c == 0){
			...
			do{
					...
					struct movesStack * UndoMarks = createStack(9);
					struct movesStack * UndoChoices = createStack(9);
					struct movesStack * RedoMarks = createStack(9);
					struct movesStack * RedoChoices = createStack(9); 
				...
				do{
					...
					if(choice == 1 && square[1] == '1'){
						...
						if(!(isEmpty(UndoChoices) && isEmpty(UndoMarks))){
						...
						if(!(isEmpty(RedoChoices) && isEmpty(RedoMarks))){
				...
				if(i == 1){
					...
					write_array_to_file(UndoChoices->movesArray, UndoMarks->movesArray); 
					reset(); 
					board();
					replay(choices,marks); 
					for(int i = 9; i >= 0; i--){
						printf("%d", UndoChoices->movesArray[i]);
					}
