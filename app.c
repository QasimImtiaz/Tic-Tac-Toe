...
void addGame(struct Game ** first_game, int game_id, char choices[9], char marks[9]){
	...
	while(1){
		...
		if(login == 0){
			...
			if(confirmed == 1){ 
				...
				int quit = 0; 
				do{
					printf("\n0 - Quit, 1 - Multiplayer Game, 2 - Play With Computer(Easy), 3 - Play With Computer(Hard), 4 - Game Replays(All Games), 5 - Game Replays(Your Games): ");
					scanf("%d", &c);
					if(c == 0){
						quit = 1; 
					}
						...
						do{
							...
							scanf("%d", &game_id);
							search(&game, game_id);
							printf("\nPress yes to search again or any key to continue: ");
							scanf("%s", &confirm); 
						}while(strcmp("%s", confirm) == 0);
...
void board()
{
	...
	printf("Press one of these numbers!\n\n");
